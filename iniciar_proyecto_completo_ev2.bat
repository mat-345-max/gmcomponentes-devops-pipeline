...
start "GM-COMPONENTS EV3 - Dashboard Streamlit 8501" cmd /k cd /d "%ROOT%ev3_observability" ^&^& "%AGENT_PYTHON%" -m streamlit run dashboard.py --server.port 8501
...
echo   EV1 groq-proxy:      http://localhost:8787/api/health
echo   EV2 FastAPI:         http://localhost:8790/health
echo   EV2 Agent (proxy):   http://localhost:8787/api/agent/health
echo   EV3 FastAPI:         http://localhost:8790/ev3/health
echo   EV3 (proxy Node):    http://localhost:8787/api/ev3/health
echo   EV3 trazas:          http://localhost:8790/ev3/traces
echo   EV3 Dashboard:       http://localhost:8501
echo.
pause