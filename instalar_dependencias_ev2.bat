...
echo.
echo ==========================================
echo Verificando observabilidad EV3
echo ==========================================
if not exist "%ROOT%ev3_observability\logs" (
  echo Creando carpeta de logs EV3...
  mkdir "%ROOT%ev3_observability\logs"
  if errorlevel 1 (
    echo.
    echo ERROR: no se pudo crear ev3_observability\logs.
    pause
    exit /b 1
  )
) else (
  echo [OK] Carpeta ev3_observability\logs ya existe.
)
"%AGENT_PYTHON%" -c "from ev3_bridge import EV3Bridge; print('EV3Bridge OK')"
if errorlevel 1 (
  echo.
  echo ERROR: no se pudo importar EV3Bridge desde agente\.venv.
  echo Revisa agente\ev3_bridge.py y las dependencias en requirements.txt.
  pause
  exit /b 1
)
echo.
echo ==========================================
echo Verificando Streamlit para dashboard EV3
echo ==========================================
"%AGENT_PYTHON%" -m pip install streamlit altair >nul 2>nul
"%AGENT_PYTHON%" -m streamlit --version >nul 2>nul
if errorlevel 1 (
  echo ERROR: streamlit no quedo instalado.
  pause
  exit /b 1
) else (
  echo [OK] Streamlit disponible para dashboard EV3.
)
echo.
echo ==========================================
