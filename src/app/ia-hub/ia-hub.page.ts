...
import { Ev3ObservabilityService } from '../services/ev3-observability.service';
import {
  Ev3Analysis,
  Ev3Metrics,
  Ev3Recommendation,
...
export class IaHubPage implements OnInit {
  ...
  selectedSegment: 'recommendation' | 'faq' | 'agents' | 'ev3' =
    'recommendation';
  faqForm!: FormGroup;
  ...
  ev3Loading = false;
  ev3Error = '';
  ev3Health: { status: string; ev3_enabled: boolean; message: string } | null =
    null;
  ev3Metrics: Ev3Metrics | null = null;
  ev3Analysis: Ev3Analysis | null = null;
  ev3Traces: Ev3Trace[] = [];
  ev3Recommendations: Ev3Recommendation[] = [];
  ev3SecurityInput = '';
  ev3SecurityResult: Ev3SecurityResult | null = null;
  ev3SecurityLoading = false;
  ev3DataLoaded = false;
  showDashboardMenu = false;
  showInlineDashboard = false;

  get ev3ErrorRate(): number {
    const total = this.ev3Analysis?.total_events ?? 0;
    const errors = this.ev3Analysis?.errors ?? 0;
    if (total === 0) return 0;
    return (errors / total) * 100;
  }

  get ev3LatencyScore(): number {
    const avg = this.ev3Analysis?.avg_latency_ms ?? 0;
    const max = this.ev3Analysis?.max_latency_ms ?? 1;
    if (max === 0) return 0;
    return Math.min(avg / max, 1);
  }

  get ev3LatencyTraces(): Ev3Trace[] {
    return this.ev3Traces
      .filter((t) => t.event === 'agent_end' && t.latency_ms != null)
      .slice(-8);
  }

  ev3IntentPercent(value: number): number {
    const entries = this.ev3IntentEntries();
    const max = Math.max(...entries.map((e) => e.value), 1);
    return (value / max) * 100;
  }

  ev3LatencyPercent(ms: number): number {
    const max = this.ev3Analysis?.max_latency_ms ?? 1;
    if (max === 0) return 0;
  ...
  constructor(
    ...
    private ev3ObservabilityService: Ev3ObservabilityService,
  ) {}
  ...
  loadEv3Data(): void {
    if (this.ev3DataLoaded) return;
    this.ev3Loading = true;
    this.ev3Error = '';

    this.ev3ObservabilityService.getHealth().subscribe({
      next: (h) => {
        this.ev3Health = h;
      },
      error: () => {
        this.ev3Error = 'No se pudo conectar con EV3.';
      },
    });

    this.ev3ObservabilityService.getMetrics().subscribe({
      next: (m) => {
        this.ev3Metrics = m;
      },
    });

    this.ev3ObservabilityService.getTraces().subscribe({
      next: (t) => {
        this.ev3Analysis = t.analysis;
        this.ev3Traces = t.traces || [];
        this.ev3Loading = false;
        this.ev3DataLoaded = true;
      },
      error: () => {
        this.ev3Loading = false;
        this.ev3Error = 'Error cargando trazas EV3.';
      },
    });

    this.ev3ObservabilityService.getRecommendations().subscribe({
      next: (r) => {
        this.ev3Recommendations = r.recommendations || [];
      },
    });
  }

  refreshEv3Data(): void {
    this.ev3DataLoaded = false;
    this.loadEv3Data();
  }
  openStreamlitDashboard(): void {
    window.open('http://localhost:8501', '_blank');
  }

  checkEv3Security(): void {
    const msg = this.ev3SecurityInput.trim();
    if (!msg) return;
    this.ev3SecurityLoading = true;
    this.ev3SecurityResult = null;

    this.ev3ObservabilityService.securityCheck(msg).subscribe({
      next: (r) => {
        this.ev3SecurityResult = r;
        this.ev3SecurityLoading = false;
      },
      error: () => {
        this.ev3SecurityLoading = false;
      },
  ...
  onSegmentChange(): void {
    if (this.selectedSegment === 'ev3') {
      this.loadEv3Data();
    }
  }

  ev3MetricEntries(): { key: string; value: unknown }[] {
    if (!this.ev3Metrics?.metrics) return [];
    return Object.entries(this.ev3Metrics.metrics).map(([key, value]) => ({
      key,
      value,
    }));
  }

  ev3IntentEntries(): { key: string; value: number }[] {
    if (!this.ev3Analysis?.intents) return [];
    return Object.entries(this.ev3Analysis.intents).map(([key, value]) => ({
      key,
      value,
    }));
  }
